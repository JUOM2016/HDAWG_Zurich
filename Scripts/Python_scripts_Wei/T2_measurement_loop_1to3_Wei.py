# -*- coding: utf-8 -*-
"""
Created on Wed Sep  7 15:18:09 2022

@author: pqslab1082: Wei 
"""
"""
...
chirpAmplitude_cleaning = 0.023 #  V; amplitude of cleaning
freq_sweeping_cleaning=0E6 # Hz; set the scanning frequency range of cleaning (the actual scanning range should be 4*freq_detuning)
cleaning_duration=0.5e-3 # s; burning time cleaning

centre_freq_pi=247.6E6 #Hz; Central frequency set to drive the AOM for pi/2 and pi pulses
...
tau_0=9e-6 # s; first time delay between pi/2 and pi pulses
tau_step=2e-6 # s; step of increasing time delay between pi/2 and pi pulses
average=100 # run the T2 measuremet loop for averaging
delay_num=20 # !!FIXED!!DO NOT CHANGE THIS!! how nany times the delay between pi/ and pi will change
echo_width=3E-6 

### Load correct Sequence file ###
HDAWG_filename = ('C:\Codes\HDAWG\Sequences\T2_measurement_loop_1to3_Wei.txt')
...
with open(HDAWG_filename, "r") as file:
    ...
    awg_program = awg_string.format(
        ...
        tau_step=tau_step, # s; step of increasing time delay between pi/2 and pi pulses
        average=average,
        delay_num=delay_num,
...
awgMod.upload_command_table(device, command_table='CT_T2_measurement_Wei')
